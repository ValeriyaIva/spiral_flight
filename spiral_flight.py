...
angle = 0.0
number_of_points = 10
increment = 360.0 / number_of_points
initial_radius = 0.6
radius_increment = 0.05  # Константа увеличения радиуса на каждой итерации
flight_height = 1.0

command_x = initial_radius * math.cos(math.radians(angle))
command_y = initial_radius * math.sin(math.radians(angle))
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if pioneer_mini.point_reached() or first_point:
            first_point = False
            if angle >= 360 * 5:  # Условие для остановки после нескольких витков спирали
                last_point_reached = True
            else:
                angle += increment
                initial_radius += radius_increment  # Увеличение радиуса на константу
                command_x = initial_radius * math.cos(math.radians(angle))
                command_y = initial_radius * math.sin(math.radians(angle))
                command_yaw = math.radians(angle)
